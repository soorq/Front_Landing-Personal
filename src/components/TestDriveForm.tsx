...
export const TestDriveForm = () => {
	...
	const onSubmit = async (data: TestDriveState) => {
		...
		try {
			setIsLoading(true);
			const res = await sendForm({ name: data.name, phone: number.join('') });
			if (!res.ok) {
				return setError(
	...
	return (
		...
		>
			<label className='block w-[220px] mb-4 mx-auto md:mb-5 md:w-[230px] xl:w-[350px] xl:ms-0'>
				...
				/>
				{errors.name && (
					<p className='w-full text-red-500 text-start'>{errors.name.message}</p>
				)}
			</label>
			...
			<label className='block w-[220px] mb-4 mx-auto md:mb-5 md:w-[230px] xl:w-[350px] '>
				...
				/>
				{errors.phone && (
					<p className='w-full text-red-500 text-start'>{errors.phone.message}</p>
				)}
			</label>

			{error && <p className='w-full text-red-500 text-start'>{error}</p>}
			...
			<label className='relative block ps-6'>
				<input
					type='checkbox'
